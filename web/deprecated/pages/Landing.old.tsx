...
import { supabase, signInWithGoogle, signOut, getUserId } from "@/lib/supabase";
...
/**
 * Carpool AI — Landing (All Warm-Dark)
 * - Full page uses warm dark gray (bg-stone-900)
 * - High-contrast, readable white/stone text
 * - No pool/buffer talk; Early Rider = launch bonus fuel
 * - Stage 1: Waitlist capture
 * - Stage 2: Logged in users see TierCards
 */

// Feature flag for Early Access mode (controlled via env var)
const EARLY_ACCESS_MODE = import.meta.env.VITE_EARLY_ACCESS === 'true';
const SHOW_WAITLIST = import.meta.env.VITE_SHOW_WAITLIST === 'true';
...
export default function Landing() {
  ...
  // Handle hash-scroll on mount and hash changes
  useEffect(() => {
    ...
    if (hash) {
      ...
      setTimeout(() => {
        ...
        if (el) {
          const headerOffset = 56; // h-14 = 56px sticky header height
  ...
  // Handle tier selection (Early Access mode only)
  const handleTierSelect = async (tier: "cruiser" | "power" | "pro") => {
    if (!userId) {
      // Not logged in → scroll to waitlist
      document.getElementById("fuel")?.scrollIntoView({ behavior: "smooth" });
      return;
    }
    ...
    try {
      ...
      const tierName = tier.charAt(0).toUpperCase() + tier.slice(1); // 'cruiser' → 'Cruiser'
  ...
  // Scroll to relevant section based on auth state
  const handleCTAClick = () => {
    if (EARLY_ACCESS_MODE && userId) {
      // Logged in + Early Access → scroll to pricing
      document.getElementById("pricing")?.scrollIntoView({ behavior: "smooth" });
    } else {
      // Public mode or not logged in → scroll to waitlist
      document.getElementById("fuel")?.scrollIntoView({ behavior: "smooth" });
    }
  };
  ...
  return (
    <main className="min-h-screen bg-stone-900 text-stone-100">
      ...
      <header className="sticky top-0 z-40 border-b border-stone-700/70 bg-stone-900/80 backdrop-blur-sm">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between">
          ...
          <nav className="hidden sm:flex items-center gap-6 text-sm text-stone-300">
            ...
            <a href="#fuel" className="hover:text-white">Early Rider</a>
      ...
      <section className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-20 sm:py-28">
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          ...
          <div className="flex flex-col gap-6">
            ...
            >
              MVP in progress · Early Riders welcome
            </motion.p>
            <motion.h1
              initial={{ opacity: 0, y: prefersReduced ? 0 : 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            <motion.div
              ...
              className="text-stone-300 text-base sm:text-lg leading-relaxed max-w-prose space-y-1"
            >
              <p>Premium AI without the premium price. Simple, fair fuel.</p>
              <p>Early Riders get a launch bonus.</p>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, y: prefersReduced ? 0 : 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              >
                {EARLY_ACCESS_MODE && userId ? "View pricing" : "Reserve my fuel"}
              </button>
              <a
                href="#demo"
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
          ...
          <ol className="grid sm:grid-cols-3 gap-6 list-decimal pl-5">
            {[
              ...
              { t: "Get launch bonus", d: "Early Riders unlock extra fuel at launch." },
            ].map((s, i) => (
      ...
      {/* PRICING (Early Access mode only) */}
      {EARLY_ACCESS_MODE && (
            ...
            <form className="mt-6 mx-auto max-w-md" onSubmit={handleWaitlistSubmit}>
              ...
              <div className="mt-3 min-h-[20px]" role="status" aria-live="polite">
                {submitStatus === "success" && (
                  <p className="text-xs text-green-400">✓ You're on the list! Check your email.</p>
                )}
          ...
          >
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-6">
              ...
              >
                ...
                <div>
                  <div className="text-sm font-medium text-white">Early Rider bonus</div>
                  <div className="text-xs text-stone-400">Extra fuel for founding members</div>
                </div>
              </motion.div>
            </div>
            <a
              href="#fuel"
              className="inline-flex rounded-xl border border-stone-700 px-4 py-2 text-sm hover:bg-stone-700 hover:scale-105 transition-all duration-200"
            >
              Get Early Rider perk
            </a>
          </motion.div>
        </div>
      </section>
      ...
      <section className="border-t border-stone-800" id="faq">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {[
              { q: "Is this live?", a: "MVP is wrapping up. Early Riders get notified first and receive a launch fuel bonus." },
              { q: "How do I use it?", a: "Sign up, pick a tier, and chat normally with visible fuel tracking." },
