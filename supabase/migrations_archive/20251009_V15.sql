...
drop trigger if exists t_u_billing on public.mvp_billing;
create trigger t_u_billing before update on public.mvp_billing
...
create index if not exists idx_usage_user_id on public.mvp_usage_logs(user_id);

-- Fuel ledger (new normalized format)
...
create index if not exists idx_fuel_user_source on public.mvp_fuel(user_id, source);

-- =========================
-- Core view for balances
-- =========================
...
-- Spend tokens (deducts in personal → reserve → pool order)
...
-- Seed allocations after Stripe purchase
...
-- Read-only per user
create policy if not exists billing_read_own
  on public.mvp_billing for select
  using (user_id = auth.uid());

create policy if not exists fuel_read_own
  on public.mvp_fuel for select
  using (user_id = auth.uid());

create policy if not exists usage_read_own
  on public.mvp_usage_logs for select
  using (user_id = auth.uid());

-- Restrict writes to server key
